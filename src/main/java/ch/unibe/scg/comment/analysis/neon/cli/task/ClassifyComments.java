...
public class ClassifyComments {
    ...
    public void run() throws SQLException {
        try (Connection connection = DriverManager.getConnection("jdbc:sqlite:" + this.database)) {
            ...
            for (int i = 0; i < buckets.size(); i = i + 1) {
                statement.executeUpdate("CREATE TABLE " + this.data + "_bucket_" + i + " (class TEXT NOT NULL REFERENCES " + this.data + "(class), PRIMARY KEY(class))");
                statement.executeUpdate("INSERT INTO " + this.data + "_bucket_" + i + " (class) VALUES " + String.join(",", buckets.get(i).stream().map(c -> "('" + c + "')").collect(Collectors.toList())));
            }
        }
    }
    ...
    private void checkPrecondition(Statement statement) throws SQLException {
        try (ResultSet result = statement.executeQuery("SELECT name FROM sqlite_master WHERE type ='table' AND name LIKE '" + this.data + "_bucket_%'")) {
            if (result.next()) {
                throw new IllegalStateException("table like " + this.data + "_bucket_%, will not perform tasks until no table like this exists to avoid overwriting data");
            }
        }
    }
